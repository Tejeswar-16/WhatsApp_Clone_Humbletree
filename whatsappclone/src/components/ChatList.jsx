...
export default function ChatList(props) {
    ...
    useEffect(() => {
        async function fetchUsers() {
            try {
                ...
                chatList = chatList.filter((cl) =>
                cl.name.toLowerCase().includes(searchName.toLowerCase())
                );
                setUsers(chatList);
                setErrorMsg("");
            } 
  ...
  return (
    <div className="flex flex-col bg-white h-screen overflow-hidden w-110 border-r border-gray-300">
      <h1 className="font-sans m-4 font-semibold text-2xl text-green-600">WhatsApp</h1>
      <input
        value={searchName}
        onChange={(e) => setSearchName(e.target.value)}
        type="text"
        className="mb-2 font-sans flex mx-auto pl-5 p-1 h-9 w-100 rounded-2xl bg-gray-100 focus:outline-none focus:ring-1 focus:ring-green-600"
            ...
            <div
              ...
              className={
                user._id === selectedChatId
                  ? "mx-3 flex flex-row rounded-xl bg-gray-100 mt-1"
                  : "mx-3 mt-1 flex flex-row rounded-xl hover:cursor-pointer hover:bg-gray-100"
              }
            >
                ...
                <div className="flex flex-row items-center mt-4 px-4">
                  ...
                  <h1 className="text-sm text-gray-500">
                    {lastMessages[user._id]?.time
                      ? new Date(lastMessages[user._id].time).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })
                      : ""}
